...
// ---------- Schedules ----------
// No auto-save on EITHER platform: edits, adds and deletes are local until you click
// the Save button, which persists everything (incl. deletes) and registers the
// Windows tasks (desktop) / reminders (mobile). The button highlights while dirty.
...
function syncLabel(dirty) {
  if (isMobile()) return dirty ? 'Save & set reminders' : 'Set phone reminders';
  return dirty ? 'Save & sync to Windows' : 'Sync to Windows now';
}
function updateSyncButton() {
  const btn = $('#sync'); if (!btn) return;
  btn.textContent = syncLabel(schedDirty);
  btn.classList.toggle('needs-save', schedDirty);
}
async function renderSchedules() {
  $('#topbar-actions').innerHTML = '<button class="btn" id="add">+ New schedule</button>'
    + `<button class="btn primary" id="sync">${syncLabel(false)}</button>`;
...
function addSchedule() {
  scheds.push({ name: 'New digest', frequency: 'weekly', time: '08:00', complexity: 'standard', focus: 'all', delivery: [], skip_if_empty: false, enabled: true });
  drawSchedules(); markSchedDirty();
}
function drawSchedules() {
  ...
  const intro = mobile
    ? `Each schedule fires a phone reminder ${tip('You get a notification at the set time (even with the app closed). Opening the app after that time runs the digest automatically and shows it in History. Tap “Set phone reminders” after editing.')} at its time. Tap “Set phone reminders” after editing. Add as many as you like — e.g. a daily “urgent” watchlist alert plus a weekly full review.`
  ...
  $('#view').querySelectorAll('[data-del]').forEach(b => b.addEventListener('click', () => {
    ...
    if (s.id) deletedSchedIds.push(s.id); // applied on Save
    scheds.splice(+b.dataset.del, 1); drawSchedules(); markSchedDirty();
  }));
...
async function syncSchedules() {
  ...
  try {
    ...
    else toast(res.ok ? 'Saved & synced to Windows Task Scheduler ✓' : 'Saved — sync had issues, see Guide', !res.ok);
  } catch (e) { toast(e.message, true); }
...
async function renderGuide() {
  ...
  $('#view').innerHTML = `<div class="guide">
    ...
    <details><summary>Automatic digests (schedules)</summary>
      <p>Create one or more schedules in the <strong>Schedules</strong> tab — each with its own frequency, time, complexity and focus.${mobile ? ' On your phone, tap <strong>Set phone reminders</strong>: each schedule fires a notification at its time (even when the app is closed). <strong>Opening the app after that time runs the digest automatically</strong> — it delivers to its channels (notification, email, Discord) and appears in History. (The digest itself needs the app open, since the analysis engine runs in the app.)' : ' Click <strong>Sync to Windows</strong> to register them with Windows Task Scheduler so they run even when the app is closed.'}</p>
      ${mobile ? '<p class="small">Allow notifications when asked. <strong>For reminders to fire when the app is closed</strong>, some phones need extra permission: turn OFF battery optimization for INVESTigator, and on Xiaomi/Redmi (MIUI) also enable <strong>Autostart</strong> (Settings → Apps → INVESTigator → Autostart, or the Security app). Without these, MIUI/Oppo/Huawei block the alarm and the reminder only appears when you next open the app. After a phone restart, open the app once to re-arm reminders.</p>' : ''}
